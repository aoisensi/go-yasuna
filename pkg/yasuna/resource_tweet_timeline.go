...
func (t *Twitter) GetUserMentions(id int64, opts ...GetUserTweetOption) (*Response[[]*Tweet], error) {
	return t.getUserTweets("mentions", id, opts...)
}

func (t *Twitter) getUserTweets(f string, id int64, opts ...GetUserTweetOption) (*Response[[]*Tweet], error) {
	path := "users/" + strconv.FormatInt(id, 10) + "/" + f
	data := new(Response[[]*Tweet])
	vi := url.Values{}
	for _, opt := range opts {
		opt.optGetUserTweet(vi)
	}
	return data, t.get(path, vi, data)
}

type GetUserTweetOption interface {
	optGetUserTweet(v url.Values)
}
